...
def simNGames(probA, probB, number):
    ...
    for game in range(number):
        ...
        if scoreA == 0: shutoutsA += 1
        elif scoreB == 0: shutoutsB += 1
...
def printOutput(winsA, winsB, shutoutsA, shutoutsB):
    n = winsA + winsB
    print('Wins for A: {0} ({1:0.1%}) with {2} shutouts ({3:0.1%})'.format(winsA, winsA/n, shutoutsA, shutoutsA/winsA))
    print('Wins for B: {0} ({1:0.1%}) with {2} shutouts ({3:0.1%})'.format(winsB, winsB/n, shutoutsB, shutoutsB/winsB))

def main():
    printIntro()
    probA, probB, n , match_type= getInput()
    if match_type == 'single-game' or match_type == 'single game' or match_type == 'SINGLE-GAME':
        winsA, winsB, shutoutsA, shutoutsB = simNGames(probA, probB, n)
    else:
        winsA, winsB, shutoutsA, shutoutsB = simNMatches(probA, probB, n, match_type)
    printOutput(winsA, winsB, shutoutsA, shutoutsB)
